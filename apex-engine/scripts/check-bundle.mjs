...
const DEFAULT_MAX_CLIENT_GZIP_KB = 260; // lightweight-charts + react-router etc.
...
for (const f of files) {
  ...
  total += gzKB;
  if (gzKB > worst.kb) worst = { name: f.slice(CLIENT_DIR.length + 1), kb: gzKB };
}

const big = files
  .map((f) => ({ name: f.slice(CLIENT_DIR.length + 1), kb: gzipSync(readFileSync(f)).length / 1024 }))
  .sort((a, b) => b.kb - a.kb)
  .slice(0, 5);

console.log(`client gzip total: ${total.toFixed(1)} kB (budget ${maxGzipKB} kB)`);
