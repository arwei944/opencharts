...
export class ScriptParser {
  ...
  parse(script: string): ScriptParseResult {
    ...
    if (cleanedScript.includes("sma(") || cleanedScript.includes("simple_average")) {
      indicatorKind = "MA";
      const periodMatch = /\(length\s*=\s*(\d+)\)/;
      const period = parseInt(periodMatch.exec(cleanedScript)?.[1] || "9");
      params.period = period;
    } else if (cleanedScript.includes("ema(") || cleanedScript.includes("exp_average")) {
      indicatorKind = "EMA";
      ...
      params.signalLength = parseInt(signalMatch.exec(cleanedScript)?.[1] || "9");
    } else {
      // Custom calculation
      ...
      return { 
        ...
        overlay: cleanedScript.includes("overlay") && cleanedScript.includes("true")
      };
    }
    
    return { success: true, indicatorKind, params, overlay: indicatorKind === "MA" || indicatorKind === "EMA" };
  }
  
  /** Parse custom calculation expressions */
  private parseCustomCalculation(script: string): ((bars: Candle[]) => Array<{ time: number; value: number }>) | null {
    try {
  ...
  private calculateRSI(bars: Candle[], period: number): Array<{ time: number; value: number }> {
